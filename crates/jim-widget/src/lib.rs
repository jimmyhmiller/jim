...
fn end_text_selection(
    mut commands: Commands,
    mut releases: MessageReader<jim_pane::PaneContentReleased>,
    mut q: Query<&mut WidgetTextSelection>,
) {
...
fn update_tooltip_hover(
    ...
    let candidates: Vec<(Entity, PaneRect)> = panes
        ...
        .map(|(e, r, _, _, _)| (e, *r))
        .collect();
    let Some(pane) = jim_pane::topmost_pane_at(pt_canvas, &candidates) else {
        return;
    };
...
fn render_text_selection_highlight(
    ...
    metrics: Res<PaneFontMetrics>,
    theme: Res<jim_style::Theme>,
    sels: Query<(&WidgetTextSelection, &jim_pane::PaneChrome)>,
    old: Query<Entity, With<WidgetSelectionHighlight>>,
) {
    for e in &old {
        commands.entity(e).try_despawn();
    }
    let base = theme.color(jim_style::tokens::ACCENT);
...
fn update_widget_hover(
    ...
    let topmost = cursor.and_then(|pt| {
        let pt_canvas = viewport.window_to_canvas(pt);
        jim_pane::topmost_pane_at(pt_canvas, &candidates).map(|e| (pt_canvas, e))
    });
