...
impl LayoutCtx {
    ...
    pub fn resolve_color(&self, s: &str) -> Option<Color> {
        if let Some(jim_style::TokenValue::Color(c)) = self.theme.get_by_name(s) {
            return Some(Color::LinearRgba(c));
        }
        jim_style::theme::parse_color_string(s)
            .ok()
    ...
    pub fn resolve_f32(&self, s: &str) -> Option<f32> {
        if let Some(jim_style::TokenValue::F32(v)) = self.theme.get_by_name(s) {
            return Some(v);
        }
        s.parse::<f32>().ok()
    }
    ...
    pub fn font_for(&self, family: &str) -> Option<Handle<Font>> {
        if let Some(jim_style::TokenValue::Str(name)) = self.theme.get_by_name(family) {
