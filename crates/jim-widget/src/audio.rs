        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                while let Ok(cmd) = rx.recv() {
                    match cmd {
                        AudioCmd::Start { device, path, dual } => {
...
/// Stop the stream and finalize the WAV. Returns the file path. The stream
/// is dropped FIRST (cpal joins its callback thread on drop), so no
/// callback can run during `finalize()`.
