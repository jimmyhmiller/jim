//! Feathers widget gallery — exploration of Bevy 0.19's `bevy_feathers`.
//!
//! Vendored, near-verbatim, from the upstream Bevy 0.19 example
//! `examples/ui/widgets/feathers_gallery.rs` (MIT/Apache-2.0, © Bevy
//! Contributors). Kept faithful on purpose: the point is to see what the real
//! authoring API feels like — the `bsn!` scene macro, `@FeathersButton`-style
//! scene-component inheritance, and `on(|ev: On<Activate>| ...)` event
//! closures. Run with `cargo run -p feathers-demo --bin feathers_demo`.
//!
//! Widgets shown: Button (+ variants, rounded-corner groups, disabled), Menu
//! (button/popup/item/divider), Checkbox, Radio (group), ToggleSwitch,
//! DisclosureToggle, Slider, TextInput, ColorSwatch / ColorPlane /
//! ColorSlider (RGB + HSL), the pane/subpane/group containers, NumberInput
//! (scalar + labelled Vec3 axes), and ListView.
        ...
        .add_plugins((
            DefaultPlugins.set(WindowPlugin {
                primary_window: Some(Window {
                    title: "Bevy 0.19 — Feathers widget gallery".into(),
                    resolution: (760u32, 900u32).into(),
                    ..default()
                }),
                ..default()
            }),
