...
//! complete token set for `theme.rhai` plus pane-chrome WGSL and a small
...
/// Write the genome as a style preset: `~/.jim/styles/<name>/theme.rhai`
...
pub fn adopt(g: &Genome, preset: &str) -> std::io::Result<PathBuf> {
    ...
    out.push_str("}\n");
    std::fs::write(dir.join("theme.rhai"), out)?;
