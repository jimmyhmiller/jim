...
/// Vertical frame-marker line: faint cool white, thin.
const MARKER_COLOR: [f32; 4] = [0.55, 0.62, 0.78, 0.35];
const MARKER_WIDTH_PX: f32 = 1.5;
...
pub struct Renderer {
    ...
    /// Optional vertical time markers (timestamps in ns) drawn as faint full-
    /// height lines across the Flame-tab timeline. Used e.g. for per-frame
    /// boundaries when viewing a recorded multi-frame buffer. Set via
...
impl Renderer {
    ...
    pub fn available_group_keys(&self) -> Vec<(StringId, String)> {
        let Some(orig) = &self.original_profile else {
            return Vec::new();
        };
    ...
    pub fn clamp_viewport(&mut self) {
        // Use the ACTIVE layout's range: in AGGREGATED (left-heavy) mode the
        // slices are laid out 0-based, so clamping against the profile's
        // absolute time range would strand them off-screen (catastrophic when
        // the trace's timestamps are large, e.g. process-epoch ns).
        let (start, end) = self.current_time_range();
    ...
    pub fn fit_all(&mut self) {
        ...
        let row_h = if total_rows == 0 {
            ...
            ((usable_h - header_total - gap_total) / total_rows as f32).clamp(0.5, ROW_HEIGHT_PX)
        };
    ...
    pub fn pan_sidebar(&mut self, dy: f32) {
        let visible =
            (self.viewport.size_px.1 - self.inspector_content_top() - STATUS_BAR_HEIGHT_PX)
                .max(0.0);
    ...
    pub fn rebuild_instances(&mut self) {
        ...
        let slices_arc: Arc<flame_core::SliceTable> =
            match (layout_mode, self.aggregated_slices.clone()) {
                (LayoutMode::LeftHeavy, Some(s)) => s,
        ...
        let use_aggregated =
            layout_mode == LayoutMode::LeftHeavy && self.aggregated_slices.is_some();
        macro_rules! slices {
            () => {
                if use_aggregated {
                    &*slices_arc
        ...
        for layout in &self.track_layouts {
            ...
            let (depth_start, depth_end) =
                visible_depth_range(direction, layout.rows, track_content_top, row_h, lo_y, hi_y);
        ...
        // Frame markers: faint full-height vertical lines at each marker time,
        // clipped to the timeline (not the inspector). Drawn over slices but
        // under the tab bar. Non-pickable (NON_SLICE_SENTINEL).
        if !self.markers.is_empty() {
            let timeline_w = (view_w_px - INSPECTOR_WIDTH_PX).max(0.0);
            let marker_top = top_offset;
            let marker_h = view_h_px.max(0.0);
            for &ns in &self.markers {
                if ns < lo_ns || ns > hi_ns {
                    continue;
                }
                let x = self.viewport.ns_to_x(ns);
                if x < 0.0 || x > timeline_w {
                    continue;
                }
                let inst_id = self.instances.len() as u32;
                self.instances.push(SliceInstance {
                    rect_px: [x, marker_top, MARKER_WIDTH_PX, marker_h],
                    color: MARKER_COLOR,
                    instance_id: inst_id,
                    flags: 0,
                    _pad: [0; 2],
    ...
    /// Set vertical time markers (ns). Caller must `rebuild_instances` after.
    pub fn set_markers(&mut self, markers: Vec<u64>) {
    ...
    fn emit_top_tab_bar(&mut self) {
        ...
        for (i, &tab) in MainTab::ALL.iter().enumerate() {
            ...
            self.slice_indices.push(NON_SLICE_SENTINEL);
            self.inspector_tab_rects
                .push((tab, [x, btn_y, btn_w, btn_h]));
    ...
    pub fn visible_tree_nodes(&self) -> Vec<(u32, bool)> {
        let Some(tree) = &self.call_tree else {
            return Vec::new();
        };
    ...
    fn emit_sequence_content(&mut self) {
        ...
        if let (Some(pk), true) = (parent_key, !span_by_id.is_empty()) {
            ...
            for i in 0..n as u32 {
                ...
                let Some(pid) = orig.attrs.get(i, pk) else {
                    continue;
                };
                let Some(&parent_i) = span_by_id.get(&pid) else {
                    continue;
                };
    ...
    fn finalize_instance_buffer(&mut self) {
        ...
        if needed_capacity > self.instance_capacity {
            ...
            self.instance_buffer = self.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("instances"),
                size: self.instance_capacity * std::mem::size_of::<SliceInstance>() as u64,
                usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            });
        }
    ...
    pub fn set_hover(&mut self, instance: Option<u32>) {
        ...
        // Read from the ACTIVE slice table: in AGGREGATED mode `slice_indices`
        // points into the aggregated table, so reading `profile.slices` would
        // report an unrelated slice's duration (the bar is sized to the
        // aggregated sum, but the hover number wouldn't match it).
        let info = instance.and_then(|id| {
            let slice_idx_raw = self
                .slice_indices
                .get(id as usize)
                .copied()
                .unwrap_or(NON_SLICE_SENTINEL);
            if slice_idx_raw == NON_SLICE_SENTINEL {
                return None;
            }
            let profile = self.profile.clone()?;
            let active = self.current_slices(&profile);
            let i = slice_idx_raw as usize;
            if i >= active.len() {
                return None;
            }
            let name = profile.strings.get(active.name[i]).to_string();
            let dur = active.dur_ns[i];
            let depth = active.depth[i];
            let track_idx = active.track[i].0 as usize;
            let track_name = profile
                .tracks
                .get(track_idx)
                .map(|t| profile.strings.get(t.name).to_string())
                .unwrap_or_else(|| "?".to_string());
            Some((name, dur, depth, track_name))
        });
        match info {
            Some((name, dur, depth, track_name)) => {
            ...
            None => self.refresh_status_for_idle(),
        }
    }
    ...
    fn prepare_text(&mut self) {
        ...
        let label_slices: &flame_core::SliceTable =
            match (self.layout_mode, self.aggregated_slices.as_ref()) {
                (LayoutMode::LeftHeavy, Some(s)) => s,
                _ => &profile.slices,
            };
        ...
        if self.active_tab == MainTab::Flame {
            ...
            for (slot, inst) in self.instances.iter().enumerate() {
                if !render_slice_labels {
                    break;
                }
                if inst.flags & 1 != 0 {
                    continue;
                }
                if inst.rect_px[2] < SLICE_LABEL_MIN_PX {
                    continue;
                }
                ...
                let name = profile
                    .strings
                    .get(label_slices.name[slice_idx_raw as usize])
                    .to_owned();
                labels.push((
                    name,
                ...
                if i < label_slices.len() {
                    ...
                    if !self.inspector_hotspot_rects.is_empty() {
                        ...
                        for (hi, rect) in self.inspector_hotspot_rects.clone().iter().enumerate() {
                            let Some((_, txt)) = row_data.get(hi) else {
                                break;
                            };
        ...
        // Close FLAME-only branch.
            ...
            for (hi, rect) in self.inspector_hotspot_rects.iter().enumerate() {
                let Some((_slice_idx, label_text)) = rows_data.get(hi) else {
                    break;
                };
        ...
        // Status bar.
        let status_y = self.viewport.size_px.1 - STATUS_BAR_HEIGHT_PX
        ...
        for (i, (text, _x, _y, max_w, metric, _color, _zone)) in labels.iter().enumerate() {
            ...
            let (fs, lh) = match metric {
                ...
                Metric::InspectorHeading => (INSPECTOR_HEADING_FONT, INSPECTOR_LINE_HEIGHT_PX),
        ...
        let text_areas: Vec<TextArea> = labels
            ...
            .map(|(i, (_, x, y, max_w, metric, color, zone))| {
                let lh = match metric {
                    ...
                    Metric::InspectorHeading | Metric::InspectorBody | Metric::MonoBody => {
                        INSPECTOR_LINE_HEIGHT_PX
                    }
    ...
    pub fn render(&mut self, view: &wgpu::TextureView) {
...

        {
            ...
            if let Err(e) =
                self.text_renderer
                    .render(&self.text_atlas, &self.glyphon_viewport, &mut pass)
            {
...
fn compute_sandwich(profile: &Profile, target: StringId) -> CallerCalleeAggregate {
    ...
    CallerCalleeAggregate {
        callers: to_list(callers),
        callees: to_list(callees),
    }
}
...
pub fn build_left_heavy_layout(
    ...
    for t_idx in 0..n_tracks {
        ...
        row_counts[t_idx] = if root_nodes.is_empty() {
            0
        } else {
            max_depth + 1
        };
...
fn build_single_track_profile(orig: &Profile) -> Profile {
    ...
    for (pos, &src) in order.iter().enumerate() {
        ...
        per_slice_attrs.push(orig.attrs.per_slice.get(src).cloned().unwrap_or_default());
    }
    if let Some(prev) = cur_depth {
...
fn slice_identity(p: &Profile, slice_idx: u32) -> Option<SliceIdentity> {
    ...
    let track_name = p
        .strings
        .get(p.tracks.get(track.0 as usize)?.name)
        .to_string();
...
fn find_slice_by_identity(p: &Profile, id: &SliceIdentity) -> Option<u32> {
    let track_id =
        p.tracks.iter().enumerate().find_map(|(i, t)| {
