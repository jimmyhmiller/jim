...
pub struct PointBinding {
    ...
    pub gap: f64,
    /// When `Some([fx, fy])`, the endpoint is anchored to a FIXED point *inside*
    /// the target — normalized to its bounding box in the unrotated frame
    /// (`(0.5, 0.5)` = centre) — instead of snapping to the edge. Set when the
    /// arrow is released inside the shape, so it can live at the centre (or any
    /// interior point) and move with the shape. `None` = normal edge binding via
    /// `focus`/`gap`.
    #[serde(default)]
