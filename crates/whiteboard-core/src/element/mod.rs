...
pub use binding::{
    apply_bound_endpoints, bindable_element_at, binding_endpoint, binding_gap, compute_binding,
    determine_focus_distance, determine_focus_point, intersect_element_with_line,
    is_bindable_element, max_binding_gap, update_bound_arrow, BoundEndpoints,
};
...
/// Recompute a linear/freedraw element's `width`/`height` and re-base its points
/// so the bbox top-left is the origin. **Call this after mutating an element's
/// points** (e.g. after a binding moves an endpoint) — otherwise `raw_box()` and
/// every coarse hit-test that uses it go stale, so you can no longer click the
/// shape the arrow points at, the selection box is wrong, etc. (Excalidraw's
/// `mutateElement` recomputes dimensions on every point change for this reason.)
pub fn resync_linear_box(el: &mut Element) {
