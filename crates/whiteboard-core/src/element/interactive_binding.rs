...
pub fn try_bind_endpoint(
    ...
    // The adjacent vertex (the neighbour of the bound endpoint) is what encodes
    // the focus direction — `which` selects which end is being dragged so we pick
    // the right neighbour. Fall back to `scene_point` for a degenerate 1-point
    // arrow (focus then resolves to 0 = aim at center).
    ...
    let adjacent = if n >= 2 {
        ...
        Point::new(arrow.x + data.points[idx].x, arrow.y + data.points[idx].y)
    } else {
        scene_point
    };
    Some(compute_binding(scene_point, adjacent, target))
}
...
mod tests {
    ...
    fn binds_endpoint_over_target() {
        ...
        // The arrow aims straight at the centre (adjacent (0,25) → end (90,25) is
        // the centre line), so focus is 0; gap 10 to the left edge (x=100).
        assert!(b.focus.abs() < 1e-9, "focus {}", b.focus);
