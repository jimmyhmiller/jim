...
use crate::geometry::{hit_test, point_rotate_rads, Point, Rect, Vec2};
...
/// onto a bindable shape when a create/drag gesture commits. A more forgiving
/// radius (and the live highlight that shows which shape will catch) makes arrow
/// attachment feel reliable instead of fiddly — the old 5px radius made it feel
/// like binding rarely worked.
const BIND_TOLERANCE: f64 = 12.0;
...
pub struct InteractionState {
    ...
    /// While drawing/dragging a linear element's free endpoint, the shape it
    /// would bind to if released now. Drives the live binding highlight; cleared
    /// when the endpoint isn't near a bindable shape or the gesture ends.
    bind_candidate: Option<ElementId>,
}
...
impl InteractionState {
    ...
    /// The shape a linear element's endpoint would bind to right now (if any),
    /// while a create/drag gesture is in progress. The editor highlights it.
    pub fn bind_candidate(&self) -> Option<&ElementId> {
        self.bind_candidate.as_ref()
    }
    ...
    fn begin_create(
        ...
        scene.insert(el);
        // Freedraw (the pen) is a fire-and-forget sketch stroke: leaving it
        // selected drops a bounding box + resize/rotation handles over every
        // stroke, which is noise when you're just drawing. Other tools stay
        // selected so you can immediately resize what you placed.
        if kind == CreateKind::Freedraw {
            self.selection.clear();
        } else {
            self.selection = std::iter::once(id.clone()).collect();
        }
    ...
    fn try_begin_vertex_drag(
        ...
        let (index, n) = {
            ...
            let verts = tools::linear_vertices_scene(el);
            let n = verts.len();
        ...
        // Grabbing a *bound* endpoint frees it so it follows the cursor — without
        // this the live binding snaps it straight back to the shape and you can't
        // pull it off. It re-binds (or stays free) on release.
        let mut changed = false;
        if index == 0 {
            unbind_one_endpoint(scene, &id, ArrowEnd::Start);
            changed = true;
        } else if n >= 2 && index == n - 1 {
            unbind_one_endpoint(scene, &id, ArrowEnd::End);
            changed = true;
        }
            ...
            Gesture::Creating {
                ...
                if let Some(el) = scene.get_mut(&id) {
                    match kind {
                        ...
                        CreateKind::Freedraw => tools::push_freedraw(el, scene_pt),
                    }
                }
                // Live binding feedback: while dragging a linear endpoint, light
                // up the shape it would attach to (binding itself happens on
                // pointer-up). Disabled when Ctrl/Cmd suppresses snapping.
                self.bind_candidate = if kind == CreateKind::Linear && !mods.ctrl && !mods.meta {
                    crate::element::bindable_element_at(scene, scene_pt, BIND_TOLERANCE)
                } else {
                    None
                };
                (
    ...
    fn on_pointer_up(
        ...
        // A gesture ending clears any active alignment guides + bind highlight.
        self.snap_guides.clear();
        self.bind_candidate = None;
        match gesture {
            Gesture::Creating {
                id, moved, kind, ..
            } => {
            ...
            Gesture::DraggingVertex { id, index, moved, .. } => {
                if moved {
                    ...
                    let n = match scene.get(&id).map(|e| &e.kind) {
                        Some(ElementKind::Arrow(d)) | Some(ElementKind::Line(d)) => d.points.len(),
                        _ => 0,
                    };
                    if index == 0 {
                        rebind_dragged_endpoint(scene, &id, ArrowEnd::Start);
                    } else if n >= 2 && index == n - 1 {
                        rebind_dragged_endpoint(scene, &id, ArrowEnd::End);
                    }
                    InteractionResult {
                        scene_changed: true,
                        view_changed: true,
...
fn topmost_at(scene: &Scene, scene_pt: Point) -> Option<ElementId> {
    ...
    for id in scene.order().iter().rev() {
        if let Some(el) = scene.get(id) {
            // Locked elements (e.g. the invisible pane proxies the canvas host
            // injects so arrows can bind to panes) are not click-selectable.
            if !el.is_deleted && !el.locked && hit_element(el, scene_pt) {
...
/// Extra erase reach (scene units at zoom 1) beyond an element's stroke, so the
/// eraser deletes on near-contact rather than requiring a pixel-perfect hit.
const ERASE_SLOP: f64 = 6.0;

/// Soft-delete **every** live element the eraser touches at `scene_pt`, recording
/// their ids in `erased` (deduped). Returns whether anything was newly erased.
///
/// Used on press and on every drag move so a single drag sweeps through
/// everything it passes over. Unlike click-selection (which uses the bbox), the
/// eraser tests against the actual outline within a tolerance — so it removes
/// thin strokes and single-point "dots" (whose bounding box is zero-area and
/// would otherwise be impossible to hit), and clears overlapping elements at
/// once instead of only the topmost.
fn erase_at(scene: &mut Scene, scene_pt: Point, erased: &mut Vec<ElementId>) -> bool {
    let hits: Vec<ElementId> = scene
        .order()
        .iter()
        .rev()
        .filter(|id| {
            scene
                ...
                .map(|el| {
                    !el.is_deleted
                        && !el.locked
                        && hit_test(el, scene_pt, el.stroke_width / 2.0 + ERASE_SLOP)
                })
                .unwrap_or(false)
        })
        .cloned()
...
/// (Re)bind or unbind a single endpoint of `arrow_id` based on whether it now sits
/// over a bindable shape — the per-endpoint version used when an arrow endpoint is
/// dragged (Excalidraw binds when you drop an end on a shape, unbinds otherwise).
fn rebind_dragged_endpoint(scene: &mut Scene, arrow_id: &ElementId, which: ArrowEnd) {
    let Some((start_pt, end_pt)) = linear_endpoints_scene(scene, arrow_id) else {
        return;
    };
    let pt = match which {
        ArrowEnd::Start => start_pt,
        ArrowEnd::End => end_pt,
    };
    if try_bind_endpoint(scene, arrow_id, which, pt, BIND_TOLERANCE).is_some() {
        bind_one_endpoint(scene, arrow_id, which, pt, BIND_TOLERANCE);
    } else {
        unbind_one_endpoint(scene, arrow_id, which);
    }
}

/// Extra reach (scene units at zoom 1) for picking thin/open elements by their
/// outline, so a diagonal arrow or freehand line can be grabbed without a
/// pixel-perfect click on the stroke.
const SELECT_SLOP: f64 = 5.0;

/// Hit-test a single element for click selection.
///
/// Open, line-like kinds (arrow / line / freedraw) are tested against their
/// actual outline within a tolerance — their bounding box is a large rectangle
/// that mostly contains empty space, so box-hit would "select" them from far off
/// the visible stroke. Closed/box kinds keep the (rotation-aware) box test, which
/// matches the filled/interior selection users expect.
fn hit_element(el: &Element, scene_pt: Point) -> bool {
    match el.kind {
        ElementKind::Arrow(_) | ElementKind::Line(_) | ElementKind::Freedraw(_) => {
            hit_test(el, scene_pt, el.stroke_width / 2.0 + SELECT_SLOP)
        }
        _ => {
