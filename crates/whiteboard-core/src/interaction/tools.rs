...
/// Append a point to a freedraw element (`cursor` in scene coords).
///
/// The new point is taken relative to the element's **current** origin, not the
/// gesture's original start. This matters because [`sync_linear_box`] re-bases
/// the point list and shifts `el.x/el.y` whenever the stroke extends past its
/// top-left (i.e. the user draws left/up). Computing the delta against a fixed
/// start would then place every subsequent point in a stale frame, accumulating
/// the rebase offset and making the stroke jump away from the cursor.
pub fn push_freedraw(el: &mut Element, cursor: Point) {
    let origin = Point::new(el.x, el.y);
    let rel = Point::new(cursor.x - origin.x, cursor.y - origin.y);
...
mod tests {
    ...
    fn freedraw_accumulates_points() {
        ...
        push_freedraw(&mut el, Point::new(10.0, 0.0));
        push_freedraw(&mut el, Point::new(10.0, 10.0));
        push_freedraw(&mut el, Point::new(10.0, 10.0)); // duplicate ignored
    ...
    /// Regression: drawing left/up past the start must keep stored scene
    /// positions aligned with the cursor (the rebase-origin bug). Each point's
    /// scene position = el.origin + stored point.
    #[test]
    fn freedraw_drawing_up_left_stays_aligned() {
        let start = Point::new(100.0, 100.0);
        let mut el = begin_element(Tool::Freedraw, ElementId::from("a"), 1, start).unwrap();
        // Cursor path: right, then back left past start, then further left/up.
        let cursors = [
        ...
        for c in cursors {
            push_freedraw(&mut el, c);
        }
        // Reconstruct scene positions and compare to the cursor path
        // (plus the initial start point).
        if let ElementKind::Freedraw(f) = &el.kind {
            let scene: Vec<Point> = f
                .points
                .iter()
                .map(|p| Point::new(el.x + p.x, el.y + p.y))
                .collect();
            let mut expected = vec![start];
            expected.extend_from_slice(&cursors);
            assert_eq!(scene.len(), expected.len());
            for (got, want) in scene.iter().zip(expected.iter()) {
                assert!(
                    (got.x - want.x).abs() < 1e-9 && (got.y - want.y).abs() < 1e-9,
                    "freedraw point {got:?} != cursor {want:?}"
                );
            }
        } else {
            panic!();
