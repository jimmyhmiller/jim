...
impl RoughGenerator {
    ...
    fn options(&self, element: &Element) -> RoughOptions {
        let mut opts = RoughOptions::for_element(element.roughness, element.seed);
        // Match Excalidraw's `generateRoughOptions`: the hachure gap scales with
        // stroke width (`strokeWidth * 4`). The fill-line *weight* also grows
        // with stroke width (`strokeWidth / 2`, applied at stroke time in the
        // tessellator). Scaling both together keeps the pattern's proportions
        // constant — without this, a fixed gap plus a thicker fill line collapses
        // the hachure into a near-solid blob at large stroke widths.
        opts.hachure_gap = (element.stroke_width * 4.0).max(1.0);
        opts
    }
}
