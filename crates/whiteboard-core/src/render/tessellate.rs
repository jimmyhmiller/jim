...
fn emit_element<G: ShapeGenerator>(
    ...
    if !element.background_color.is_transparent() {
        ...
        // color. Match Excalidraw's `fillWeight = strokeWidth / 2` — the fill
        // line is half the outline weight (and the gap scales with stroke width
        // in `RoughGenerator::options`, so the pattern stays proportional).
        if !fill_strokes.is_empty() {
            let fill_stroke = Stroke::solid((element.stroke_width / 2.0).max(0.5));
