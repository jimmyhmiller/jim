...
///
/// A single point yields one degenerate (zero-length) segment so distance tests
/// still see the point — without this a one-point freedraw "dot" has no
/// outline at all and can never be hit (e.g. the eraser can't remove it).
fn open_chain_segments(pts: &[Point]) -> Vec<(Point, Point)> {
    if pts.len() == 1 {
        return vec![(pts[0], pts[0])];
    }
