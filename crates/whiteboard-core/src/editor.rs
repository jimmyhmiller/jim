...
impl<M: TextMeasurer, G: ShapeGenerator> Editor<M, G> {
    ...
    /// Mutable access to the scene. Backends use this to stamp freshly-created
    /// elements with the active style (color/width/fill) — the per-gesture undo
    /// snapshot is taken in [`Editor::handle`] before creation, so styling a
    /// just-created element within the same gesture is captured correctly.
    ...
    pub fn handle(&mut self, event: InputEvent) -> HandleResult {
        ...
        // Keep bound arrows attached *live*: any scene-mutating event (a move or
        // resize mid-drag, not just the final commit) re-derives bound endpoints
        // from their targets' current positions. Without this the arrow stays put
        // while you drag the shape and only jumps to the edge on release — which
        // looks like it "snaps to a corner" at the end instead of following.
        // Cheap: only arrows that actually carry a binding are touched.
    ...
    fn refresh_bindings(&mut self) {
        ...
        for id in bound_arrows {
            ...
            if let Some(arrow) = self.scene.get_mut(&id) {
                ...
                // Moving an endpoint changes the element's extent — recompute its
                // box so hit-testing/selection stay correct (otherwise the arrow's
                // stale bbox covers the shape it points at and you can't click it).
                if changed {
                    crate::element::resync_linear_box(arrow);
                }
            }
        }
    }
    ...
    pub fn render_with_overlay_style(&self, style: &crate::render::OverlayStyle) -> RenderScene {
        ...
        // Excalidraw shows a single selected arrow/line as just its endpoint
        // dots — NOT a bounding-box-with-resize-handles. Drawing that box around
        // every arrow (and around a just-drawn arrow, which auto-selects) is the
        // "annoying box". So suppress the box+handle layout for a lone linear
        // selection; the vertex-handle pass below draws its endpoints instead.
        let single_linear = self.selection().len() == 1
            && self
                .selection()
                .iter()
                .next()
                .and_then(|id| self.scene.get(id))
                .map(|el| !crate::interaction::linear_vertices_scene(el).is_empty())
                .unwrap_or(false);
        let layout = if single_linear {
            None
        } else {
            self.interaction.handle_layout(&self.scene, &self.viewport)
        };
        ...
        // Live binding highlight: while dragging a linear endpoint near a
        // bindable shape, outline that shape so the user sees what the arrow will
        // attach to on release. In scene space → map to screen.
        if let Some(cand) = self.interaction.bind_candidate() {
            if let Some(el) = self.scene.get(cand) {
                use crate::geometry::{element_bounds, Path, Point as P, Rect};
                use crate::render::{Color, DrawCommand, Paint, Stroke};
                let s = to_screen.apply_rect_bounds(&element_bounds(el));
                let pad = 3.0;
                let r = Rect::new(
                    s.min_x() - pad,
                    s.min_y() - pad,
                    (s.max_x() - s.min_x()) + 2.0 * pad,
                    (s.max_y() - s.min_y()) + 2.0 * pad,
                );
                ...
                scene.push(DrawCommand::StrokePath {
                    ...
                    paint: Paint::solid(Color::rgba(0x42, 0x63, 0xeb, 0xdd)),
                });
            }
        }

        // Snapping alignment guides: thin, soft violet segments shown while a
        // move snaps to another element. In scene space → map to screen. Kept
        // subtle (low-saturation violet, partial alpha) so they read as a hint
        // rather than a harsh magenta flash.
        let guides = self.interaction.snap_guides();
        if !guides.is_empty() {
            use crate::geometry::Path;
            use crate::render::{Color, DrawCommand, Paint, Stroke};
            let violet = Color::rgba(0x8b, 0x5c, 0xf6, 0xb0);
            for g in guides {
                let a = to_screen.apply(g.a);
                let b = to_screen.apply(g.b);
                scene.push(DrawCommand::StrokePath {
                    path: Path::polyline(&[a, b]),
                    stroke: Stroke::solid(0.75),
                    paint: Paint::solid(violet),
                });
            }
        }
