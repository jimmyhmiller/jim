...
fn drag_endpoint_onto_box_binds_it() {
    ...
    assert!(
        end_bound_to(&ed, &arrow).is_none(),
        "ends in empty space → unbound"
    );
    ...
    assert_eq!(
        end_bound_to(&ed, &arrow),
        Some(ElementId::from("box")),
        "dropping the end on the box binds it"
    );
...
fn drag_bound_endpoint_off_unbinds() {
    ...
    assert!(
        end_bound_to(&ed, &arrow).is_none(),
        "dragging the end into empty space unbinds it"
    );
}
