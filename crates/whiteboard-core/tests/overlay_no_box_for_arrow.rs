...
fn arrow_selection_has_no_bounding_box() {
    ...
    assert!(
        rect_extra >= 8,
        "a selected rectangle should draw a box + handles, got {rect_extra}"
    );
    ...
    assert!(
        arrow_extra > 0,
        "a selected arrow should still show endpoint handles"
    );
    assert!(
    ...
    assert!(
        arrow_extra <= 6,
        "expected only endpoint handles for an arrow, got {arrow_extra}"
    );
}
