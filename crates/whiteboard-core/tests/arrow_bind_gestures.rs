...
fn edge_binding_keeps_box_clickable_and_follows() {
    ...
    assert!(
        arrow_end(&ed, &arrow).x < 300.0,
        "endpoint snapped outside the edge"
    );
    assert_eq!(
    ...
    assert!(
        (box_el.x - 420.0).abs() < 1.0,
        "box moved +120, x={}",
        box_el.x
    );
    let end = arrow_end(&ed, &arrow);
    assert!(
        end.x > 380.0 && end.x < 470.0,
        "endpoint followed to the moved edge, end={end:?}"
    );
}
...
fn interior_binding_lives_at_center_and_follows() {
    ...
    let end = arrow_end(&ed, &arrow);
    assert!(
        (end.x - 350.0).abs() < 2.0 && (end.y - 230.0).abs() < 2.0,
        "endpoint at centre, end={end:?}"
    );
    ...
    assert!(
        d.end_binding.as_ref().unwrap().fixed_point.is_some(),
        "interior fixed-point binding"
    );
    ...
    assert!(
        ed.selection().contains(&ElementId::from("box")),
        "box clickable away from the arrow"
    );
    ed.handle(down(320.0, 212.0));
    ...
    let end = arrow_end(&ed, &arrow);
    assert!(
        (end.x - 470.0).abs() < 2.0 && (end.y - 230.0).abs() < 2.0,
        "endpoint followed to new centre, end={end:?}"
    );
}
