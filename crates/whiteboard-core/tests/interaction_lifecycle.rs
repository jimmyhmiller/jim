...
/// A freedraw stroke drawn up/left past its start keeps its rendered points
/// aligned with the cursor path (regression for the rebase-origin bug that made
/// the pen "jump off screen") and leaves nothing selected (so no selection box
/// frames the stroke).
#[test]
fn freedraw_up_left_tracks_cursor_and_does_not_select() {
    let mut ed = editor();
    ed.set_tool(Tool::Freedraw);
    let path = [
        (100.0, 100.0),
        (110.0, 100.0),
        (90.0, 100.0),
    ...
    ed.handle(down(path[0].0, path[0].1));
    for p in &path[1..] {
        ed.handle(mv(p.0, p.1));
    }
    ...
    // Reconstruct scene positions; they must match the cursor path.
    let scene: Vec<(f64, f64)> = pts.iter().map(|p| (el.x + p.x, el.y + p.y)).collect();
    assert_eq!(scene.len(), path.len());
    for (got, want) in scene.iter().zip(path.iter()) {
        assert!(
            (got.0 - want.0).abs() < 1e-9 && (got.1 - want.1).abs() < 1e-9,
            "freedraw scene point {got:?} != cursor {want:?}"
        );
    }
...
/// no outline segments otherwise and the eraser would skip it.
#[test]
fn eraser_removes_a_freedraw_dot() {
    let mut ed = editor();
    // Tap with the pen to leave a dot.
    ed.set_tool(Tool::Freedraw);
    ed.handle(down(200.0, 150.0));
    ed.handle(up(200.0, 150.0));
    assert_eq!(ed.scene().iter_live().count(), 1, "a tap leaves a dot");

    // Erase a few px away from the dot — within tolerance.
    ed.set_tool(Tool::Eraser);
    ed.handle(down(203.0, 151.0));
    ed.handle(up(203.0, 151.0));
    assert_eq!(
        ed.scene().iter_live().count(),
        0,
        "eraser should remove the dot"
    );
}
