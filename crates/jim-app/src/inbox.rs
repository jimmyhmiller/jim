...
fn handle_content_press(
    ...
    mut panes: Query<(&mut InboxPane, &jim_pane::PaneChrome)>,
...
fn update_inbox_hot_zones(
    panes: Query<(Entity, &InboxPane, &jim_pane::PaneChrome)>,
...
fn sync_inbox_content_root(
    inboxes: Query<(&InboxPane, &jim_pane::PaneChrome)>,
    ...
    for (pane, chrome) in &inboxes {
        ...
        let want_y = -(jim_pane::TITLE_H + jim_pane::MARGIN) + pane.scroll;
...
fn rebuild_rows(
    ...
    mut panes: Query<(&mut InboxPane, &PaneRect, &jim_pane::PaneChrome)>,
    existing_rows: Query<(Entity, &ChildOf), With<InboxRowEntity>>,
    store: Res<InboxStore>,
    font: Res<PaneFont>,
    font_metrics: Res<jim_pane::PaneFontMetrics>,
