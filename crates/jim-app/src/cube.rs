...
fn overview_apply(
    ...
    (style_state, preset_registry, style_data_dir): (
        Res<jim_style::ProjectStyleState>,
        Res<jim_style::presets::StylePresetRegistry>,
...
fn project_theme_bg(
    pid: u64,
    style_state: &jim_style::ProjectStyleState,
    preset_registry: &jim_style::presets::StylePresetRegistry,
    style_data_dir: Option<&jim_style::StyleDataDir>,
) -> Color {
    let path = style_state
        ...
        .or_else(|| style_data_dir.map(|d| jim_style::theme::theme_path_for_project(d, pid)));
    let bg = path
        .filter(|p| p.exists())
        .and_then(|p| jim_style::theme::load_theme(&p).ok())
        .map(|t| t.color(jim_style::tokens::BG))
        .unwrap_or_else(|| jim_style::Theme::default().color(jim_style::tokens::BG));
    Color::LinearRgba(bg)
}
