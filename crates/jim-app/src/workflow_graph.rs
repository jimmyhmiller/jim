...
use bevy::prelude::*;
use jim_pane::{PaneKindMarker, PaneKindSpec, PaneRect, PaneRegistry, PaneTitle};
...
fn drain_and_render(
    ...
    pane_font: Option<Res<jim_pane::PaneFont>>,
    ...
    for (kind, rect, mut pane, mut view) in &mut q {
        ...
        let content_size = Vec2::new(
            (rect.size.x - 2.0 * jim_pane::MARGIN).max(0.0),
            (rect.size.y - jim_pane::TITLE_H - 2.0 * jim_pane::MARGIN).max(0.0),
        );
...
fn palette(theme: &jim_style::Theme) -> GraphPalette {
