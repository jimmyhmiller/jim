...
fn run_button_palette(theme: &jim_style::Theme) -> RunButtonPalette {
...
fn run_button_spawn_from_config(
    ...
    let font = world
        .get_resource::<jim_pane::PaneFont>()
        .map(|f| f.0.clone())
...
fn update_run_button_hot_zones(
    ...
    for (kind, rect, rb, mut zones) in &mut q {
        ...
        zones.clear();
        let content_w = (rect.size.x - 2.0 * jim_pane::MARGIN).max(0.0);
...
fn output_lines_capacity(rect: &PaneRect) -> usize {
    let content_h = (rect.size.y - jim_pane::TITLE_H - 2.0 * jim_pane::MARGIN).max(0.0);
...
fn sync_run_button_visual(
    ...
    buttons: Query<(&RunButton, &RunButtonChrome, &jim_pane::PaneRect)>,
